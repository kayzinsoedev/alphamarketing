    ...
    <thead>
      <tr>
		<?php foreach ($columns as $col) { ?>
			<td <?php if(in_array($col, array('quantity'))){ ?>style="width:30px"<?php } ?>><?php echo $language->get('column_'.$col); ?></td>
		<?php } ?>
      ...
      <tr>
		<?php foreach ($columns as $col) { ?>
			<td <?php if(in_array($col, array('weight', 'quantity', 'price', 'tax', 'tax_rate', 'tax_total', 'price_tax', 'total'))){ ?>class="right"<?php } ?> <?php if(in_array($col, array('image'))){ ?>style="width:1px"<?php } ?>>
				<?php if($col == 'product'){ ?>
					<?php /*if(true || isset($prod_options['quantity'])){echo $product['quantity'].' x ';}*/ ?><?php echo $product['name']; ?> <?php if($product['redeem'] == '1'){echo "<span style='color:red'>(Redeem)</span>";} ?>
