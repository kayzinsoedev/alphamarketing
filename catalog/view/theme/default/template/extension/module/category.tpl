...
<div id="filter-groups">
    ...
    <!--<?= $filters; ?>-->
    <?= $prices; ?>
    <?php if($category_layout == '2' || $category_layout == '3'){ ?>
        <?php include_once(DIR_TEMPLATE.'default/template/product/sort_order_2.tpl'); ?>
    <?php } ?>
    <!-- filter length -->
    <?= $length; ?>
    <!-- filter length -->
    ...
    <script type="text/javascript">
        ...
        function applyFilter(updatePrice){
            ...
            <?php if($category_layout == '1'){ ?>
                if( $('#input-sort').length && $('#input-sort').val() ) url += '&sort=' + $('#input-sort').val();
                if( $('#input-limit').length && $('#input-limit').val() ) url += '&limit=' + $('#input-limit').val();
            <?php }else{ ?>
                if ($(window).width() > 767) {
                    if( $('#input-sort2').length && $('#input-sort2').val() ) url += '&sort=' + $('#input-sort2').val();
                    if( $('#input-limit2').length && $('#input-limit2').val() ) url += '&limit=' + $('#input-limit2').val();
                }else{
                    if( $('#input-sort').length && $('#input-sort').val() ) url += '&sort=' + $('#input-sort').val();
                    if( $('#input-limit').length && $('#input-limit').val() ) url += '&limit=' + $('#input-limit').val();
                }
