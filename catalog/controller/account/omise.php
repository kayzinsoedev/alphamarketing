		...
		if (isset($this->request->get['id'])) {
    		...
    		if ($omise_customer_query->num_rows) {
        		...
        		if ($omise_keys) {
        		    ...
        		    $customer = OmiseCustomer::retrieve($this->request->get['id']);
        		    
        		  //  $card = $customer->getCards()->retrieve($this->request->get['id']);
                    
                    $customer->destroy();
        		    
        		    if($customer->isDestroyed()){
            			...
            			if ($this->config->get('config_customer_activity')) {
            				...
            				$this->model_extension_payment_omise->removeOmiseCustomer($this->request->get['id']);
            			}
...
// 			$omise_customer = OmiseCustomer::retrieve();
			
// 		    foreach($omise_customer_query->rows AS $id => $omise_customer_result) {
		        
// 		        $vv = array_search($omise_customer_result['omise_customer_id'], array_column($omise_customer['data'], 'id'));
		        
// 		        $omise_customer_all_card = $omise_customer['data'][$vv]['cards']['data'];
		        
//                 foreach ($omise_customer_all_card as $result) {
//                     $data['credit_cards'][] = array(
//         				'id' => $result['id'],
//         				'omise_card_no' => '('. $result['name'] .') Card no. ending with '. $result['last_digits'],
//         				'delete'     => $this->url->link('account/omise/delete', 'id=' . $omise_customer['data'][$vv]['id'], true)
//         			);
//                 }
// 		    }
// 		}
