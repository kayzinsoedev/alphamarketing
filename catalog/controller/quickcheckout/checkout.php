...
class ControllerQuickCheckoutCheckout extends Controller {
	...
	public function index() {
		...
		if($this->config->get('setting_facebookevents_status') == 1){
			...
			foreach($products as $fb_products) {			
				$fbq_items[] = array(
					'id' => $fb_products['product_id'],
					'name' =>  $fb_products['name'],
					'quantity' => $fb_products['quantity'],
					'price' => $fb_products['price'],
				);
				
				$data['num_items'] = $data['num_items'] + $fb_products['quantity'];
				$fbq_total_value = $fbq_total_value + ($fb_products['quantity'] * $fb_products['price']);
			}
