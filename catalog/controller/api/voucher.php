...
class ControllerApiVoucher extends Controller {
	public function index() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			$voucher_info = $this->model_extension_total_voucher->getVoucher($voucher,$order_id);
	...
	public function send() {
		...
		// select all records which meet current date
		$order_voucher_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_voucher WHERE order_id > '0' AND delivery_date = '".date('Y-m-d')."'");
	
		$this->load->model('checkout/order');
		foreach ($order_voucher_query->rows as $voucher) {
			$this->model_checkout_order->sendVoucher($voucher['voucher_id']);
		}
