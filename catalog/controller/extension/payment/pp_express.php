		...
		if ((string)$response == "VERIFIED")  {
			...
			if ($transaction) {
				...
				if ($transaction['payment_status'] != $this->request->post['payment_status']) {
					...
					// Update order history & send payment received email
					$this->load->model('checkout/order');
					if($this->config->get('config_payment_received_status')) {
						$payment_received_id = $this->config->get('config_payment_received_status');
					}
					else {
						$payment_received_id = 17;
					}
					$this->model_checkout_order->addOrderHistory($transaction['order_id'], $payment_received_id, '', 1, 0);
