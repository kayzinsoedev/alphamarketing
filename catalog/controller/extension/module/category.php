...
class ControllerExtensionModuleCategory extends Controller {
	...
	public function index() {
		...
		if($this->config->get('category_ctgrs_status')){
			...
			if($this->config->get('theme_default_product_category_layout_setting') == "2" || $this->config->get('theme_default_product_category_layout_setting') == "3"){
				$data['categories'] = $this->getCategoryFilter2();
			}else{
		...
		foreach($limits as $key => $value) {
			if($key == 0){
				$data['limits'][] = array(
					'text'  => $this->language->get('text_default_view'),
					'value' => $value,
				);
			}else{
				$data['limits'][] = array(
	...
	private function getCategoryFilter(){
		...
		if($this->config->get('theme_default_product_category_layout_setting') == "2" || $this->config->get('theme_default_product_category_layout_setting') == "3"){
			...
			if(!isset($this->request->get['path'])){
				$data['category_active'] = true;
			}
			$data['all_category'] = $this->url->link('product/category');
		}
	...
	//layout 2
	private function getCategoryFilter2(){
		...
		if($path[0] == 0){
			...
			$categories = $this->model_catalog_category->getCategories($path[0]);
			if(!$categories){
				$categories = $this->model_catalog_category->getCategories(0);
			}
		}
