				...
				foreach ($results as $result) {
					...
					if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
		                if ($this->config->get('config_product_decimal_places')) {
							...
							$price = $this->currency->format2($this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax')), $currency_code);
		                }
					} else {
						$price = false;
					}
					...
					if ((float)$result['special']) {
		                if ($this->config->get('config_product_decimal_places')) {
							...
							$special = $this->currency->format2($this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax')), $currency_code);
		                }
					} else {
						$special = false;
					}
