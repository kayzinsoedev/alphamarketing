...
class ModelExtensionPaymentFomopay extends Model {
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int)$this->config->get('fomopay_geo_zone_id') . "' AND `country_id` = '" . (int)$address['country_id'] . "' AND (`zone_id` = '" . (int)$address['zone_id'] . "' OR `zone_id` = '0')");

		if ($this->config->get('fomopay_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('fomopay_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				...
				'terms'      => '',
				'sort_order' => $this->config->get('fomopay_sort_order')
			);
		}
	...
	public function addOrder($order_data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "fomopay_order` SET
	...
	public function addTransaction($transaction_data) {
		/**
		 * 1 to many relationship with fomopay order table, many transactions per 1 order
		 */

		$this->db->query("INSERT INTO `" . DB_PREFIX . "fomopay_order_transaction` SET
			`fomopay_order_id` = '" . (int)$transaction_data['fomopay_order_id'] . "',
	...
	public function paymentRequestInfo() {
		...
		$data['PAYMENTREQUEST_0_PAYMENTACTION'] = $this->config->get('fomopay_transaction');
	...
	public function getTotalCaptured($fomopay_order_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `" . DB_PREFIX . "fomopay_order_transaction` WHERE `fomopay_order_id` = '" . (int)$fomopay_order_id . "' AND `pending_reason` != 'authorization' AND `pending_reason` != 'paymentreview' AND (`payment_status` = 'Partially-Refunded' OR `payment_status` = 'Completed' OR `payment_status` = 'Pending') AND `transaction_entity` = 'payment'");

		return $qry->row['amount'];
	}

	public function getTotalRefunded($fomopay_order_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `" . DB_PREFIX . "fomopay_order_transaction` WHERE `fomopay_order_id` = '" . (int)$fomopay_order_id . "' AND `payment_status` = 'Refunded'");

		return $qry->row['amount'];
	}

	public function getTransactionRow($transaction_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "fomopay_order_transaction` `pt` LEFT JOIN `" . DB_PREFIX . "fomopay_order` `po` ON `pt`.`fomopay_order_id` = `po`.`fomopay_order_id`  WHERE `pt`.`transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
	...
	public function updateOrder($capture_status, $order_id) {
		$this->db->query("UPDATE `" . DB_PREFIX . "fomopay_order` SET `date_modified` = now(), `capture_status` = '" . $this->db->escape($capture_status) . "' WHERE `order_id` = '" . (int)$order_id . "'");
	}

	public function call($data) {
		if ($this->config->get('fomopay_test')) {
			$api_url = 'https://api-3t.sandbox.fomopay.com/nvp';
			$api_user = $this->config->get('fomopay_sandbox_username');
			$api_password = $this->config->get('fomopay_sandbox_password');
			$api_signature = $this->config->get('fomopay_sandbox_signature');
		} else {
			$api_url = 'https://api-3t.fomopay.com/nvp';
			$api_user = $this->config->get('fomopay_username');
			$api_password = $this->config->get('fomopay_password');
			$api_signature = $this->config->get('fomopay_signature');
		}
	...
	public function log($data, $title = null) {
		if ($this->config->get('fomopay_debug')) {
			$this->log->write('fomopay Express debug (' . $title . '): ' . json_encode($data));
		}
	}
