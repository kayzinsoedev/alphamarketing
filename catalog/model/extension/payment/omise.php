...
class ModelExtensionPaymentOmise extends Model {
    ...
    public function getCustomerByID($id) {
        return $this->db->query("SELECT * FROM `" . DB_PREFIX . "omise_customer` WHERE id = '" . (int)$id. "'");
    }
    
    public function addOmiseCustomer($customer_id, $email, $cust_id) {
        return $this->db->query("INSERT INTO " . DB_PREFIX . "omise_customer SET customer_id = '".(int)$customer_id."', omise_customer_id = '".$this->db->escape($cust_id)."', date_added = NOW()");		
    }
    
    public function removeOmiseCustomer($cust_id) {
        return $this->db->query("DELETE FROM " . DB_PREFIX . "omise_customer WHERE omise_customer_id = '".$this->db->escape($cust_id)."'");
