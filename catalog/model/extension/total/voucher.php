...
class ModelExtensionTotalVoucher extends Model {
	...
	public function getVoucher($code,$order_id = 0) {
		$status = true;
		...
		if ($voucher_query->num_rows) {
			...
			if($order_id > 0){
				$voucher_history_query = $this->db->query("SELECT SUM(amount) AS total FROM `" . DB_PREFIX . "voucher_history` vh WHERE vh.voucher_id = '" . (int)$voucher_query->row['voucher_id'] . "' AND vh.order_id != '".$order_id."' GROUP BY vh.voucher_id");
			}else{
	...
	public function getTotal($total) {
		if (isset($this->session->data['voucher'])) {
			...
			$voucher_info = $this->getVoucher($this->session->data['voucher'],$order_id);
