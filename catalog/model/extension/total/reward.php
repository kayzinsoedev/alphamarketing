...
class ModelExtensionTotalReward extends Model {
	public function getTotal($total) {
		if (isset($this->session->data['reward'])) {
			...
			if(isset($total['order_id'])){
				$point_history = $this->db->query("SELECT * FROM ".DB_PREFIX."customer_reward WHERE order_id = '".$total['order_id']."'");
				if($point_history->num_rows){
					$point_use = abs($point_history->row['points']);
				}else{
					$point_use = 0;
				}
				$points = $points + $point_use;
			}
	...
	public function confirm($order_info, $order_total) { 
		...
		if($order_info['order_id']){
			$order_product_id = $this->db->query("SELECT * FROM ".DB_PREFIX."order_product WHERE order_id = '".(int)$order_info['order_id']."' AND redeem = '1' AND redeem_point > 0");
			foreach($order_product_id->rows as $order_product){
				$points += (int)($order_product['redeem_point'] * $order_product['quantity']);
			}
		}
		...
		if ($this->model_account_customer->getRewardTotal($order_info['customer_id']) >= $points) {
			$reward_history = $this->db->query("SELECT * FROM ".DB_PREFIX."customer_reward WHERE order_id = '" . (int)$order_info['order_id']."' AND points < 0");
			if(!$reward_history->num_rows){
