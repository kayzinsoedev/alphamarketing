		...
		foreach ($settings['charge'] as $charge) {
			...
			foreach ($cart_products as $product) {
				...
				if($product['product_id'] == 0){
					$product_query->row['quantity'] = 1000;
					$product_query->row['manufacturer_id'] = 0;
					$product_query->row['price'] = $product['price'];
				}
			...
			$charges[strtolower($charge['group'])][] = array(
				'title_admin'	=> $charge['title_admin'],
