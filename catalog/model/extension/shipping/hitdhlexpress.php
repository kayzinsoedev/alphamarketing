...
class ModelExtensionShippinghitdhlexpress extends Model {
	function getQuote($address) {
		...
		if($this->config->get('hitdhlexpress_realtime_rates') == true)
		{
			$status = true;
		}
		else
		...
		if ($status) {
			...
			$dhl_country = $this->config->get('hitdhlexpress_country_code');
			...
			if (!empty($cart_products)) {
				...
				foreach ($cart_products as $prod) {
					...
					//$this->weight->getUnit();
					$op_prod_weight_cls_id = $prod['weight_class_id'];
					$op_prod_dim_cls_id = $prod['length_class_id'];

					$mod_weight_unit = ($this->config->get('hitdhlexpress_weight') == true) ? 'lb' : 'kg';
					$mod_dim_unit = ($this->config->get('hitdhlexpress_weight') == true) ? 'in' : 'cm';
...
// echo '<pre>'; print_r($weight_query);die();
					...
					if (!empty($mod_weight_cls_id) && ($op_prod_weight_cls_id != $mod_weight_cls_id)) {
						...
						$prod['weight'] = round($this->weight->convert( $prod['weight'], $op_prod_weight_cls_id, $mod_weight_cls_id ), 2);
					}
					if (!empty($mod_dim_cls_id) && ($op_prod_dim_cls_id != $mod_dim_cls_id)) {
						$prod['length'] = round($this->length->convert( $prod['length'], $op_prod_dim_cls_id, $mod_dim_cls_id ), 2);
						$prod['width'] = round($this->length->convert( $prod['width'], $op_prod_dim_cls_id, $mod_dim_cls_id ), 2);
						$prod['height'] = round($this->length->convert( $prod['height'], $op_prod_dim_cls_id, $mod_dim_cls_id ), 2);
					}
			...
			// print_r($prducts);echo '<h4>before.....<h4><br/>';print_r($cart_products); die();
			...
			if (!$this->config->get('hitdhlexpress_test')) {
			...
			$weight_unit = ($this->config->get('hitdhlexpress_weight') == true) ? 'LB' : 'KG';
			$dim_unit = ($this->config->get('hitdhlexpress_weight') == true) ? 'IN' : 'CM';
			$fetch_accountrates = ($this->config->get('hitdhlexpress_rate_type') == 'ACCOUNT') ? "<PaymentAccountNumber>" . $this->config->get('hitdhlexpress_account') . "</PaymentAccountNumber>" : "";
		
			$mailing_date = date('Y-m-d');
			$mailing_datetime = date('c');
			$origin_postcode_city = $this->hit_get_postcode_city($this->config->get('hitdhlexpress_country_code'), $this->config->get('hitdhlexpress_city'), $this->config->get('hitdhlexpress_postcode'));
			//$total_value = $this->cart->get_total();
			$is_dutiable = ($address['iso_code_2'] == $this->config->get('hitdhlexpress_country_code') || $this->hit_dhl_is_eu_country($this->config->get('hitdhlexpress_country_code'), $address['iso_code_2'])) ? "N" : "Y";
			...
			$xml .= '					<SiteID>'.$this->config->get('hitdhlexpress_key').'</SiteID>';
			$xml .= '					<Password>'.$this->config->get('hitdhlexpress_password').'</Password>';
			$xml .= '			</ServiceHeader>';
			$xml .= '		</Request>';
			$xml .= '		<From>';
			$xml .= '			<CountryCode>'.$this->config->get('hitdhlexpress_country_code').'</CountryCode>';
			$xml .= '			'.$origin_postcode_city;
			$xml .= '		</From>';
			$xml .= '		<BkgDetails> ';
			$xml .= '			<PaymentCountryCode>'.$this->config->get('hitdhlexpress_country_code').'</PaymentCountryCode>';
			...
			$result = utf8_encode(curl_exec($curl));
				...
				$result = $xml;		
			if($this->config->get('hitdhlexpress_display_time') == true)
			{
				echo "<PRE>";
				echo "<h2>Request<br/>";
				print_r(htmlspecialchars($reuest));
				echo "<br/><h2>Response<br/>";
				print_r($result);
				die();	//	die();	
			}
			if ($result && !empty($result->GetQuoteResponse->BkgDetails->QtdShp)) {
				...
				foreach ($result->GetQuoteResponse->BkgDetails->QtdShp as $quote) {
					...
					$selected_services_aaray = $this->config->get('hitdhlexpress_service');
...
//echo "<pre>";		//	print_r($result);		//	die();
		...
		//	}
			...
			$method_data = array(
				...
				'title'      => $title,
				'quote'      => $quote_data,
				'sort_order' => $this->config->get('hitdhlexpress_sort_order'),
				'error'      => $error
			);
		}
		return $method_data;
	...
	{
		switch ($this->config->get('hitdhlexpress_packing_type')) {
			case 'weight_based' :
	...
	public function weight_based_shipping($package,$orderCurrency,$chk='')
		{
			$maximum_weight = ($this->config->get('hitdhlexpress_wight_b') !='') ? $this->config->get('hitdhlexpress_wight_b') : '50';
			...
			if( !empty($errors) ){
				...
				foreach($packages as $package){//pre($package);
					...
					$group = array(
						...
						'Units' => ($this->config->get('hitdhlexpress_weight') == true) ? 'LBS' : 'KG'
					),
	...
	private function per_item_shipping($package,$orderCurrency,$chk = false) {
		...
		foreach ($package as $item_id => $values) {
			...
			if($values['weight'] < 0.001){
				...
				$dhl_per_item_weight = $values['weight'] = round(($values['weight']/$values['quantity']), 3);
			}
			...
			if ($values['width'] && $values['height'] && $values['length']) {
				...
				$group['Dimensions'] = array(
					...
					'Units' => ($this->config->get('hitdhlexpress_weight') == true) ? 'IN' : 'CM'
				);
			}
			$group['packtype'] = $this->config->get('hitdhlexpress_per_item');
			$group['InsuredValue'] = $insurance_array;
