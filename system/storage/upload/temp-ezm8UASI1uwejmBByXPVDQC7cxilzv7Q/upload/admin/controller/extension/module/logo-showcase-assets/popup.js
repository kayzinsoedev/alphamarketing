(function($) {
    ...
    let $deactivateForm  = jQuery('#logo-showcase-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('logo-showcase-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'logo-showcase-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('logo-showcase-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_logo_showcase.php', jQuery.param(data)).then(function(result) {
