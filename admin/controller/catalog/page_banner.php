...
class ControllerCatalogPagebanner extends Controller {
	...
	protected function getRouteSets(){
		...
		$service_array[] = array(
			...
			'route' =>	'service/scategory',
			'query' =>	'*',
			'value'	=>	'service/scategory-*',
		);
		// ); $scategories = $this->getScategories();
		// foreach($scategories as $scategory){ 
		// 	$service_array[] = array(
		// 		'name'	=>	$scategory['name'],
		// 		'route' =>	'service/scategory',
		// 		'query'	=>	'scat=' . $scategory['scategory_id'],
		// 		'value'	=>	'service/scategory-' . 'ncat=' . $scategory['scategory_id'],
		// 	);
		// }
		...
		// News / Blog Article
		$sarticle_array = array();
		$sarticle_array[] = array(
			'name'	=>	'All Service',
			'route' =>	'service/article',
			'query' =>	'*',
			'value'	=>	'service/article-*',
		); $article = $this->getSArticle();

		//debug($article);

		$data[] = array(
			'optgroup'	=>	'Services Articles',
			'options'	=>	$sarticle_array
		);
	...
	public function getScategories(){
		$query = $this->db->query('SELECT DISTINCT scategory_id, name FROM `'. DB_PREFIX . 'sb_scategory_description` WHERE language_id="'.(int)$this->config->get('config_language_id').'" ORDER BY name ASC');
		return $query->rows;
	}

	public function getSArticle(){
		$query = $this->db->query('SELECT DISTINCT service_id, title as name FROM `'. DB_PREFIX . 'sb_service_description` WHERE language_id="'.(int)$this->config->get('config_language_id').'" ORDER BY title ASC');
		return $query->rows;
	}
