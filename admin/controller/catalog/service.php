...
class ControllerCatalogService extends Controller {
	...
	public function index() {
		$this->language->load('catalog/service');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			...
			if ($this->config->get('service_archive_build')) {
				$this->buildArchive();
			}
	...
	public function copy() { 
		...
		if (isset($this->request->post['delete']) && $this->validateCopy()) {
			foreach ($this->request->post['delete'] as $service_id) {
	...
	private function getList() {
		...
		$data['insert'] = $this->url->link('catalog/service/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
		$data['npages'] = $this->url->link('module/service', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['copy'] = $this->url->link('catalog/service/copy', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data['entry_sauthor'] = $this->language->get('entry_sauthor');
		
		$article_limit = $this->config->get('ncategory_bservice_admin_limit') ? $this->config->get('ncategory_bservice_admin_limit') : 20;
		...
		$results = $this->model_catalog_service->getServiceLimited($sdata);
    	...
    	foreach ($results as $result) {
			...
			$data['service'][] = array(
				...
				'delete'      => isset($this->request->post['delete']) && in_array($result['service_id'], $this->request->post['delete']),
				'action'      => $action
			);
		}
		...
		$data['sort_title'] = $this->url->link('catalog/service', 'token=' . $this->session->data['token'] . '&sort=nd.title' . $url, 'SSL');
		
		$data['sort_author'] = $this->url->link('catalog/service', 'token=' . $this->session->data['token'] . '&sort=na.name' . $url, 'SSL');
		                            
		$data['sort_sort_order'] = $this->url->link('catalog/service', 'token=' . $this->session->data['token'] . '&sort=n.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/service', 'token=' . $this->session->data['token'] . '&page={page}' . $url, 'SSL');;
		$data['pagination'] = $pagination->render();
		
		$data['results'] = sprintf($this->language->get('text_pagination'), ($service_total) ? (($page - 1) * $article_limit) + 1 : 0, ((($page - 1) * $article_limit) > ($service_total - $article_limit)) ? $service_total : ((($page - 1) * $article_limit) + $article_limit), $service_total, ceil($service_total / $article_limit));
		...
		$data['servicepanel'] = $this->load->controller('common/servicepanel');
	...
	private function getForm() {
		...
		$data['bservice_html_editor'] = $this->config->get('ncategory_bservice_html_editor');
	...
	if (isset($this->error['warning'])) {
		...
		if (!isset($this->request->get['service_id'])) {
			$data['action'] = $this->url->link('catalog/service/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
			
		} else {
			$data['action'] = $this->url->link('catalog/service/update', 'token=' . $this->session->data['token'] . '&service_id=' . $this->request->get['service_id'] . $url, 'SSL');
		...
		if ((isset($this->request->get['service_id'])) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$service_info = $this->model_catalog_service->getServiceStory($this->request->get['service_id']);
		}
		...
		$authors = $this->model_catalog_sauthor->getAuthors();
		...
		} elseif (isset($service_info) && $service_info['gal_thumb_w']) {
			$data['gal_thumb_w'] = $service_info['gal_thumb_w'];
		} else {
			$data['gal_thumb_w'] = 150;
		}
		if (isset($this->request->post['gal_thumb_h'])) {
			$data['gal_thumb_h'] = $this->request->post['gal_thumb_h'];
		} elseif (isset($service_info) && $service_info['gal_thumb_h']) {
			$data['gal_thumb_h'] = $service_info['gal_thumb_h'];
		} else {
			$data['gal_thumb_h'] = 150;
		}
		if (isset($this->request->post['gal_popup_w'])) {
			$data['gal_popup_w'] = $this->request->post['gal_popup_w'];
		} elseif (isset($service_info) && $service_info['gal_popup_w']) {
			$data['gal_popup_w'] = $service_info['gal_popup_w'];
		} else {
			$data['gal_popup_w'] = 700;
		}
		if (isset($this->request->post['gal_popup_h'])) {
			$data['gal_popup_h'] = $this->request->post['gal_popup_h'];
		} elseif (isset($service_info) && $service_info['gal_popup_h']) {
			$data['gal_popup_h'] = $service_info['gal_popup_h'];
		} else {
			$data['gal_popup_h'] = 700;
		}
		if (isset($this->request->post['gal_slider_h'])) {
			$data['gal_slider_h'] = $this->request->post['gal_slider_h'];
		} elseif (isset($service_info) && $service_info['gal_slider_h']) {
			$data['gal_slider_h'] = $service_info['gal_slider_h'];
		} else {
			$data['gal_slider_h'] = 400;
		}
		if (isset($this->request->post['gal_slider_w'])) {
			$data['gal_slider_w'] = $this->request->post['gal_slider_w'];
		} elseif (isset($service_info) && $service_info['gal_slider_w']) {
			$data['gal_slider_w'] = $service_info['gal_slider_w'];
		} else {
			$data['gal_slider_w'] = 980;
		}
		if (isset($this->request->post['gal_slider_t'])) {
			$data['gal_slider_t'] = $this->request->post['gal_slider_t'];
		} elseif (isset($service_info) && $service_info['gal_slider_t']) {
			$data['gal_slider_t'] = $service_info['gal_slider_t'];
		} else {
			$data['gal_slider_t'] = 1;
		}
		...
		if (isset($this->request->post['date_added'])) {
			...
			$data['date_added'] = $service_info['date_added'];
		} else {
		...
		if (isset($this->request->post['date_pub'])) {
			...
			$data['date_pub'] = $service_info['date_pub'];
		} else {
		...
		if (isset($this->request->post['sauthor_id'])) {
			$data['sauthor_id'] = $this->request->post['sauthor_id'];
		} elseif (isset($service_info)) {
			$data['sauthor_id'] = $service_info['sauthor_id'];
		} else {
			$data['sauthor_id'] = '';
		}
		...
		if (isset($this->request->post['image2'])) {
			...
			$data['image2'] = $service_info['image2'];
		} else {
			$data['image2'] = '';
		}
		...
		if (!empty($service_info) && $service_info['image'] && file_exists(DIR_IMAGE . $service_info['image'])) {
			$data['thumb'] = $this->model_tool_image->resize($service_info['image'], 100, 100);
		} else {
			$data['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		if (!empty($service_info) && $service_info['image2'] && file_exists(DIR_IMAGE . $service_info['image2'])) {
			$data['thumb2'] = $this->model_tool_image->resize($service_info['image2'], 100, 100);
		} else {
		...
		if (isset($this->request->post['service_group'])) {
			$data['service_group'] = $this->request->post['service_group'];
		} elseif (isset($this->request->get['service_id'])) {
			$data['service_group'] = $this->model_catalog_service->getServiceGroups($this->request->get['service_id']);
		} else {
			$ng=array(); for($i=1; $i<151; $i++) $ng[] = $i;
			$data['service_group'] = $ng;
		}	
		...
		if (isset($this->request->post['service_ncategory'])) {
			$data['service_ncategory'] = $this->request->post['service_ncategory'];
		} elseif (isset($this->request->get['service_id'])) {
			$data['service_ncategory'] = $this->model_catalog_service->getServiceNcategories($this->request->get['service_id']);
		} else {
			$data['service_ncategory'] = array();
		}	
		
		if (isset($this->request->post['service_nrelated'])) {
			$nrelated = $this->request->post['service_nrelated'];
		} elseif (isset($this->request->get['service_id'])) {		
			$nrelated = $this->model_catalog_service->getServiceNrelated($this->request->get['service_id']);
		} else {
			$nrelated = array();
		}
		
		$data['service_nrelated'] = array();
		foreach ($nrelated as $narticle_id) {
			$article_related_info = $this->model_catalog_service->getServiceStory($narticle_id);
			
			if ($article_related_info) {
				$data['service_nrelated'][] = array(
					'service_id' => $article_related_info['service_id'],
		...
		if (isset($this->request->post['service_related'])) {
			$products = $this->request->post['service_related'];
		} elseif (isset($this->request->get['service_id'])) {		
			$products = $this->model_catalog_service->getServiceRelated($this->request->get['service_id']);
		} else {
			$products = array();
		}
		...
		// Gallery
		if (isset($this->request->post['service_gallery'])) {
			$service_gallery = $this->request->post['service_gallery'];
		} elseif (isset($this->request->get['service_id'])) {
			$service_gallery = $this->model_catalog_service->getArticleImages($this->request->get['service_id']);
		} else {
			$service_gallery = array();
		}

		$data['service_gallery'] = array();

		foreach ($service_gallery as $service_image) {
			...
			$data['service_gallery'][] = array(
				...
				'text'       => $service_image['text'],
				'sort_order' => $service_image['sort_order']
			);
		}
		
		// Videos
		if (isset($this->request->post['service_video'])) {
			$service_video = $this->request->post['service_video'];
		} elseif (isset($this->request->get['service_id'])) {
			$service_video = $this->model_catalog_service->getArticleVideos($this->request->get['service_id']);
		} else {
			$service_video = array();
		}

		$data['service_video'] = array();

		foreach ($service_video as $video) {
			$data['service_video'][] = array(
				'text'       => $video['text'],
				'video'      => $video['video'],
				'width'      => $video['width'],
	...
	private function validateAuthor() {
		...
		$author_id = $this->model_catalog_service->getAuthorIdbyArticle($this->request->get['service_id']);
		
		if ($author_id ) {
			$adminid = $this->model_catalog_sauthor->getAuthorAdminID($author_id);
	...
	protected function validateDelete() {
		...
		if (!$this->error) {
			...
			foreach ($this->request->post['delete'] as $service_id) {
				...
				$author_id = $this->model_catalog_service->getAuthorIdbyArticle($service_id);
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_aname'])) {
			...
			$results = $this->model_catalog_service->getServiceLimited($data);
	...
	private function buildArchive() {
	  if ($this->validateCopy()) {
		...
		$restr = $this->config->get('ncategory_bservice_restrictgroup');
		
		$stores = $this->model_setting_store->getStores();
		
		$this->language->load('module/service_archive');
		  
		if ($this->config->get('service_archive_date') == "dp") {
			$articles = $this->model_catalog_service->getServiceDP(); //get all articles with date published
			$field = 'date_pub'; // archive grouping field set to date_pub
		} elseif($this->config->get('service_archive_date') == "du") {
			$articles = $this->model_catalog_service->getServiceDU(); //get all articles with date updated
			$field = 'date_updated'; // archive grouping field set to date update
		} else {
			$articles = $this->model_catalog_service->getServiceDA(); //get all articles
			$field = 'date_added'; // archive grouping field
		}
		
		
		$this->model_catalog_service->deleteArchive(); //clear the archive
	...
	foreach($customer_groups as $group) {
		...
		if ($archive_data) {
			...
			foreach ($archive_data as $archive) {
				$this->model_catalog_service->addArchiveYear($archive, $rid);
			}
		}
		...
		if ($archive_data) {
			foreach ($archive_data as $archive) {
				$this->model_catalog_service->addArchiveYear($archive);
			}
		}
		/*end of default store*/
		...
		foreach ($stores as $store) {
			...
			if ($archive_data) {
				foreach ($archive_data as $archive) {
					$this->model_catalog_service->addArchiveYear($archive, (int)$store['store_id']);
				}
			}
		}
}
