...
class ControllerExtensionShippingFormulaBased extends Controller {
...
//    private $name = 'complex';
	...
	public function loadSettings(&$data) {
        ...
        $this->cache->delete('complex');
        unset($this->session->data['complex']);
	...
	public function saveSettings() {
		...
		if ($this->request->get['saving'] == 'manual') {
            ...
            $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `code` = '" . $this->db->escape($code_) . "' AND `key` != '" . $this->db->escape('complex_module') . "'");
        }
		...
		foreach ($this->request->post as $key => $value) {
			if (strpos($key, 'module_') === 0) {
                ...
                if ($this->request->get['saving'] == 'auto') {
                    ...
                    $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE `code` = 'complex' AND `key` = '" . $this->db->escape($key_) . "'");
                }
