...
class ControllerEcadvancedreportsSaleState extends Ec_Report_Abstract { 
	public function index() {   
		$this->initLoad();
		$data = $this->_data;
		$data['export'] = '';

		$this->template = 'module/ecadvancedreports/sale_state.tpl';
	...
	public function initLoad($all_records = false) {
		...
		$this->_data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('ecadvancedreports/sale_state', 'token=' . $this->session->data['token'].$url, 'SSL'),
			'separator' => ' :: '
		);
		...
		if($results) {
			foreach($results as $key => $result) {
				if($result) {
					...
					$results[$key]['link'] = $this->url->link('ecadvancedreports/sale_state_detail', 'zone_id='.$result['zone_id'].'&zone_name='.urlencode($result['zone_name']).$url.'&token=' . $this->session->data['token'], 'SSL');
				}
		...
		$this->_data['heading_title'] = $this->language->get('heading_title_sales_state');
		...
		$this->_data['text_show_report_detail'] = $this->language->get('text_show_report_detail');
		...
		$pagination->url = $this->url->link('ecadvancedreports/sale_state', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
