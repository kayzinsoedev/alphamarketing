...
class ControllerEcadvancedreportsSaleStateDetail extends Ec_Report_Abstract { 
	public function index() {   
		$this->initLoad();
		$data = $this->_data;
		$data['export'] = '';

		$this->template = 'module/ecadvancedreports/sale_state_detail.tpl';
	...
	public function initLoad($all_records = false) {
		...
		if (isset($this->request->get['zone_name'])) {
			$zone_name = $this->request->get['zone_name'];
			$zone_name = urldecode($zone_name);
		} else {
			$zone_name = "";
		}
		...
		$heading_title = $this->language->get('heading_title_sales_state_detail');
		$heading_title = str_replace("%s", $zone_name, $heading_title);
		...
		if (isset($this->request->get['zone_name'])) {
			$url .= '&zone_name=' . urlencode($this->request->get['zone_name']);
		}
		...
		$this->_data['breadcrumbs'][] = array(
			'text'      => $heading_title,
			'href'      => $this->url->link('ecadvancedreports/sale_state_detail', 'token=' . $this->session->data['token'].$url, 'SSL'),
			'separator' => ' :: '
		);
		...
		$data = array(
			...
			'filter_zone_name'		 => $zone_name,
			'start'                  => ($page - 1) * $limit,
			'limit'                  => $limit,
			'all'		=> $all_records
		);

		$report_total = $this->getModel()->getTotalSaleStateDetail($data);
		...
		if($results) {
			foreach($results as $key => $result) {
				if($result) {
					...
					$results[$key]['customer_url'] = $this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'], 'SSL');
					$results[$key]['order_url'] = $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'], 'SSL');
				}
		...
		$this->_data['heading_title'] = $this->language->get('heading_title_sales_state_detail');
		$this->_data['heading_title'] = str_replace("%s", $zone_name, $this->_data['heading_title']);
		...
		$this->_data['column_added_date'] = $this->language->get('column_added_date');
		...
		$this->_data['column_state'] = $this->language->get('column_state');
		...
		$this->_data['zone_id'] = $zone_id;

		$this->_data['zone_name'] = $zone_name;
		...
		$pagination->url = $this->url->link('ecadvancedreports/sale_state_detail', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	public function export() {
		...
		$reports = array();
		$reports['name'] = 'report_sale_state_page_'.$page;
		$reports['data'] = array();
		$reports['data'] = array();
	...
	protected function queryReports( $data ){
		...
		return $this->getModel()->getSaleByStateDetail($data);
	}
}
?>