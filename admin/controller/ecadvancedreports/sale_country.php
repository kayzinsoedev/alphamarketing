...
class ControllerEcadvancedreportsSaleCountry extends Ec_Report_Abstract {
	public function index() {
		...
		$this->template = 'module/ecadvancedreports/sale_country.tpl';
	...
	public function initLoad() {
		...
		$this->document->setTitle($this->language->get('heading_title_sale_by_country'));
		...
		$this->_data['breadcrumbs'][] = array(
			'text'      => $this->language->get('heading_title_sale_by_country'),
			'href'      => $this->url->link('ecadvancedreports/sale_country', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
		...
		$results = $this->getModel()->getSaleByCountry($data);
		$sum_total = 0;
		$sum_qty = 0;

		if($results) {
			foreach($results as $key => $result) {
				$sum_total += (float)$result['order_total'];
				$sum_qty += (float)$result['order_quantity'];
		...
		$this->_data['heading_title'] = $this->language->get('heading_title_sale_by_country');
