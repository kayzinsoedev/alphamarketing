...
class ControllerEcadvancedreportsSaleDayWeek extends Ec_Report_Abstract {
	public function index() {
		...
		$this->template = 'module/ecadvancedreports/sale_day_week.tpl';
	...
	public function initLoad() {
		...
		$this->document->setTitle($this->language->get('heading_title_sale_by_day_week'));
		...
		$this->_data['breadcrumbs'][] = array(
			'text'      => $this->language->get('heading_title_sale_day_week'),
			'href'      => $this->url->link('ecadvancedreports/sale_day_week', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
		...
		$dates = array(
				6 => $this->language->get("text_sunday"),
				0 => $this->language->get("text_monday"),
				1 => $this->language->get("text_tuesday"),
				2 => $this->language->get("text_wednesday"),
				3 => $this->language->get("text_thursday"),
				4 => $this->language->get("text_friday"),
				5 => $this->language->get("text_saturday"),
				);

		$results = $this->getModel()->getSaleByDayWeek($data);
		...
		foreach($dates as $key => $date) {
			if(isset($tmp[$key])){
				$results[$tmp[$key]]['date'] = $date;
				$reports[] = $results[$tmp[$key]];

			} else {
				$reports[] = array("quantity" => 0,
										"total" => 0.00,
		...
		$this->_data['heading_title'] = $this->language->get('heading_title_sale_by_day_week');
		...
		$this->_data['column_day_of_week'] = $this->language->get('column_day_of_week');
