...
class ControllerEcadvancedreportsSaleProductCountry extends Ec_Report_Abstract {
	public function index() {
		...
		$this->template = 'module/ecadvancedreports/sale_product_country.tpl';
	...
	public function initLoad() {
		...
		$this->document->setTitle($this->language->get('heading_title_sale_product_per_country'));
		...
		$this->_data['breadcrumbs'][] = array(
			'text'      => $this->language->get('heading_title_sale_product_per_country'),
			'href'      => $this->url->link('ecadvancedreports/sale_product_country', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
		...
		$results = $this->getModel()->getSaleProductPerCountry($data);
		$sum_total = 0;
		$sum_qty = 0;
		$sum_order_total = 0;
		...
		if($results) {
			foreach($results as $key => $result) {
				$sum_total += (float)$result['total_product'];
				$sum_qty += (float)$result['total_qty_product'];
				$sum_order_total += (float)$result['order_total'];
				$results[$key]['total2'] = $this->currency->format($result['order_total'], $this->config->get('config_currency'));
			}
		}
		...
		$this->_data['sum_order_total'] = $sum_order_total;
		$this->_data['sum_order_total_with_currency'] = $this->currency->format($sum_order_total, $this->config->get('config_currency'));

		$this->_data['heading_title'] = $this->language->get('heading_title_sale_product_country');
		...
		$this->_data['column_number_product'] = $this->language->get('column_number_product');
		$this->_data['column_quantity'] = $this->language->get('column_product_quantity');
		...
		$this->_data['key_list'] = array("qty" => $this->language->get('column_quantity'),
										...
										"total_product" => $this->language->get('column_total_product'));
	...
	public function export() {
		...
		$reports = array();
		$reports['name'] = 'sale_product_country';
		$reports['data'] = array();
