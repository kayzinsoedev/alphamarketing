...
class ControllerEcadvancedreportsSaleCustomer extends Ec_Report_Abstract { 
	public function index() {   
		$this->initLoad();
		$this->data['export'] = '';
		$this->template = 'module/ecadvancedreports/sale_customer.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function initLoad($all_records = false) {
		...
		$this->document->setTitle($this->language->get('heading_title_sale_customer'));
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => $this->language->get('heading_title_customer_city'),
			'href'      => $this->url->link('ecadvancedreports/customer_city', 'token=' . $this->session->data['token']. $url, 'SSL'),
			'separator' => ' :: '
		);
		...
		$report_total = $this->getModel()->getTotalCustomerCity($data);

		$results = $this->getReports( $data, "city");
		...
		$this->data['sum_customers'] = $sum_customers;
		...
		$this->data['sum_refunded'] = $sum_refunded;
		...
		$this->data['sum_refunded_with_currency'] = $this->currency->format($sum_refunded, $this->config->get('config_currency'));

		$this->data['heading_title'] = $this->language->get('heading_title_customer_city');
		...
		$this->data['text_filter_store'] = $this->language->get('text_filter_store');
		...
		$this->data['column_refunded'] = $this->language->get('column_refunded');
		...
		$this->data['detail_key_list'] = array("0" => $this->language->get('text_grouped'),
										"1" => $this->language->get('text_detailed'));

		$this->data['export_types'] = $this->get_export_types();

		$this->data['range_list'] = array(
		...
		$pagination->url = $this->url->link('ecadvancedreports/customer_city', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	public function export() {
		...
		if($export_type == "pdf" || $export_type == "html") {
			...
			$this->template = 'module/ecadvancedreports/customer_city.tpl';
		...
		$reports = array();
		$reports['name'] = 'report_customer_city_page_'.$page;
		$reports['data'] = array();
	...
	protected function getReports( $data = array(), $report_period = "city") { 
		return $this->queryReports($data);
	}
	protected function queryReports( $data ){
		
		return $this->getModel()->getCustomerByCity($data);
	}
}
?>