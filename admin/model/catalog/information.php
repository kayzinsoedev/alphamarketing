...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', template_type = '" . $this->db->escape($data['template_type']) . "', status = '" . (int)$data['status'] . "'");
		...
		//Insert Information Repeater Row
		$this->db->query("DELETE FROM " . DB_PREFIX . "information_repeater WHERE information_id = '" . (int)$information_id . "'");
		
		if (isset($data['information_repeater'])) {

			foreach ($data['information_repeater'] as $language => $information) {
				foreach ($information as $information_repeater) {
					$row_type = '';
					if(isset($information_repeater['row_type']) && $information_repeater['row_type']) {
						$row_type = $information_repeater['row_type'];
					}
				
					// // Removes duplicates
					// $this->db->query("DELETE FROM " . DB_PREFIX . "information_repeater WHERE information_id = '" . (int)$information_id . "' AND information_repeater_id = '" . (int)$information_repeater['information_repeater_id'] . "'");
					
					$this->db->query("INSERT INTO " . DB_PREFIX . "information_repeater SET image = '" . $this->db->escape($information_repeater['image']) ."', information_id = '" . (int)$information_id . "', row_type = '" . $row_type . "', language_id = '" . (int)$language . "', description = '" .  $this->db->escape($information_repeater['description']['text']) . "'");
		
					// foreach ($information_repeater['description'] as $language_id => $information_repeater_description) {
					// 	// debug($information_repeater_description);die;
					// 	// $this->db->query("DELETE FROM " . DB_PREFIX . "information_repeater WHERE information_id = '" . (int)$information_id . "' AND information_repeater_id = '" . (int)$information_repeater['information_repeater_id'] . "' AND language_id = '" . (int)$language_id . "'");

					// 	$this->db->query("INSERT INTO " . DB_PREFIX . "information_repeater SET language_id = '" . (int)$language_id . "', description = '" .  $this->db->escape($information_repeater['description']['text']) . "'");
					// }
				}
	...
	public function editInformation($information_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', template_type = '" . $this->db->escape($data['template_type']) . "', status = '" . (int)$data['status'] . "' WHERE information_id = '" . (int)$information_id . "'");
	...
	public function getInformationRepeater($information_id) {
		...
		$information_repeater_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information_repeater WHERE information_id = '" . (int)$information_id. "'");
		...
		return $information_repeater_data;
	}

	public function getInformationsbyType($type = '') {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) LEFT JOIN " . DB_PREFIX . "information_to_store i2s ON (i.information_id = i2s.information_id) WHERE i.template_type='".$this->db->escape($type)."' AND id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND i2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND i.status = '1' ORDER BY i.sort_order, LCASE(id.title) ASC");

		return $query->rows;
	}

	public function getTemplateType($type = '') {
		$sql = "SELECT * FROM " . DB_PREFIX . "template_type";

		if(isset($type) && $type) {
			$sql .= " WHERE template_type='".$this->db->escape($type)."'";
		}
		$query = $this->db->query($sql);
