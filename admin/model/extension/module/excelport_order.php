...
class ModelExtensionModuleExcelportorder extends ModelExtensionModuleExcelport {
	public function importXLSOrders($file, $importLimit = 100) {
		...
		$map = array(
			...
			'payment_country'			=> 21,
			'payment_zone'				=> 22,
			'payment_address_format'	=> 23,
			'payment_method'			=> 24,
			'payment_code'				=> 25,
			'shipping_firstname'		=> 26,
			'shipping_lastname'			=> 27,
			'shipping_company'			=> 28,
			'shipping_address_1'		=> 29,
			'shipping_address_2'		=> 30,
			'shipping_unit_no'			=> 31,
			'shipping_city'				=> 32,
			'shipping_postcode'			=> 33,
			'shipping_country'			=> 34,
			'shipping_zone'				=> 35,
			'shipping_address_format'	=> 36,
			'shipping_method'			=> 37,
			'shipping_code'				=> 38,
			'comment'					=> 39,
			'total'						=> 40,
			...
			'delivery_time'				=> 56,
			'custom_field'				=> 57,
			'payment_custom_field'		=> 58,
			'shipping_custom_field'		=> 59,
			'marketing_id'				=> 60,
			'tracking'					=> 61
		);
		...
		do {
			...
			if ($order_store_id !== '') {
				...
				$order = array(
					...
					'payment_unit_no'			=> strval(trim($orderSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['payment_unit_no']) . ($source[1]))->getValue())),
					...
					'shipping_unit_no'			=> strval(trim($orderSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['shipping_unit_no']) . ($source[1]))->getValue())),
					...
					'delivery_date'					=> strval(trim($orderSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['delivery_date']) . ($source[1]))->getValue())),
					'delivery_time'					=> strval(trim($orderSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['delivery_time']) . ($source[1]))->getValue())),
	...
	public function exportXLSOrders($destinationFolder = '', $orderNumber, $export_filters = array()) {
		...
		$generals = array(
			...
			'payment_city'				=> 19,
			'payment_postcode'			=> 20,
			'payment_country'			=> 21, // Need to be outputted in the Meta sheet
			'payment_zone'				=> 22, // Needs to be a valid value
			'payment_address_format'	=> 23,
			'payment_method'			=> 24,
			'payment_code'				=> 25,
			'shipping_firstname'		=> 26,
			...
			'shipping_city'				=> 32,
			'shipping_postcode'			=> 33,
			'shipping_country'			=> 34, // Need to be outputted in the Meta sheet
			'shipping_zone'				=> 35, // Needs to be a valid value
			'shipping_address_format'	=> 36,
			'shipping_method'			=> 37,
			'shipping_code'				=> 38,
			'comment'					=> 39,
			'total'						=> 40,
			'order_status_id'			=> 41, // Need to be outputted in the Meta sheet
			'affiliate_id'				=> 42, // Need to be outputted in the Meta sheet
			'commission'				=> 43,
			'language_id'				=> 44, // Need to be outputted in the Meta sheet
			'currency_id'				=> 45, // Need to be outputted in the Meta sheet
			'currency_code'				=> 46, // Need to be outputted in the Meta sheet
			'currency_value'			=> 47,
			'ip'						=> 48,
			'forwarded_ip'				=> 49,
			'user_agent'				=> 50,
