    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-location" class="form-horizontal">
          ...
          <div class="form-group">
            <label class="col-sm-2 control-label" for="input-region">Regions</label>

            <div class="col-sm-10">
              <select name="region" id="input-region" class="form-control">
                <?php foreach ($regions as $single_region) { ?>
                <?php if ($single_region['name'] == $region) { ?>
                <option value="<?= $single_region['name']; ?>" selected="selected"><?= $single_region['name']; ?></option>
                <?php } else { ?>
                <option value="<?= $single_region['name']; ?>"><?= $single_region['name']; ?></option>
                <?php } ?>
                <?php } ?>
              </select>
            </div>
          ...
          <div class="form-group">
            <label class="col-sm-2 control-label" for="input-gmap_iframe"><?php echo $entry_gmap_iframe; ?></label>
            <div class="col-sm-10">
              <textarea name="gmap_iframe" rows="5" placeholder="<?php echo $entry_gmap_iframe; ?>" id="input-gmap_iframe" class="form-control"><?php echo $gmap_iframe; ?></textarea>
            </div>
          </div>
          <div class="form-group">
