...
<form action="" method="post" enctype="multipart/form-data" id="form-blog">
    ...
    <table class="table table-bordered table-hover">
		...
		<tbody>
			<?php if ($service) { ?>
				<?php $class = 'odd'; ?>
				<?php foreach ($service as $service_story) { ?>
					<?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
					<tr class="<?php echo $class; ?>">
						<td align="text-center" style="padding-top: 4px;">
							<?php if ($service_story['delete']) { ?>
								<input type="checkbox" name="delete[]" value="<?php echo $service_story['service_id']; ?>" checked="checked" />
							<?php } else { ?>
								<input type="checkbox" name="delete[]" value="<?php echo $service_story['service_id']; ?>" />
							<?php } ?>
						</td>
						<td class="text-left"><?php echo $service_story['title']; ?></td>
						<td class="text-right"><?php echo $service_story['status']; ?></td>
						<td class="text-right">
							<?php foreach ($service_story['action'] as $action) { ?>
...
$('#button-filter').on('click', function() {
	var url = 'index.php?route=catalog/service&token=<?php echo $token; ?>';
