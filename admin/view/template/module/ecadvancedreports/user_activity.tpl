        ...
        <table class="list list-reports table table-bordered table-hover">
          <thead>
            <tr>
               ...
               <td class="text-left"><?php echo $column_new_accounts; ?></td>
               <td class="text-left"><?php echo $column_number_orders; ?></td>
               <td class="text-left"><?php echo $column_reviews; ?></td>
            </tr>
          </thead>
          <tbody>
            ...
            <?php if($report) { ?>
              <tr>
                ...
                <td class="text-right"><?php echo isset($report["number_customers"])?$report['number_customers']:0;?></td>
                <td class="text-right"><?php echo isset($report["number_orders"])?$report['number_orders']:0;?></td>
                <td class="text-right"><?php echo isset($report["number_reviews"])?$report['number_reviews']:0;?></td>
              </tr>
              <?php } else { ?>
              <tr class="not_found">
                <td class="text-left"><?php echo $key; ?></td>
          ...
          <tfoot>
            <tr>
              ...
              <td class="text-right"><b><?php echo $sum_customers; ?></b></td>
              <td class="text-right"><b><?php echo $sum_orders;?></b></td>
              <td class="text-right"><b><?php echo $sum_reviews;?></b></td>
            </tr>
          </tfoot>
        </table>
      <?php } else { ?>
...
function ecExport() {
  url = 'index.php?route=ecadvancedreports/user_activity/export&token=<?php echo $token; ?>';
...
function filter() {
  url = 'index.php?route=ecadvancedreports/user_activity&token=<?php echo $token; ?>';
...
<script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['<?php echo $language->get("text_date");?>','<?php echo $column_new_accounts; ?>', '<?php echo $column_number_orders; ?>', '<?php echo $column_reviews; ?>'],
          <?php
            $tmp = array();
            foreach($reports as $key=>$val) {
                if($val) {
                  $tmp[] = "['".$key."', ".(isset($val["number_customers"])?$val["number_customers"]:"0").", ".(isset($val["number_orders"])?$val["number_orders"]:"0").", ".(isset($val["number_reviews"])?$val["number_reviews"]:"0")."]";
                }
            }
           echo implode(",", $tmp);
          ?>
