...
<div id="content">
  ...
  <div class="container-fluid">
    ...
    <div class="content report_earnings panel panel-default">
      ...
      <div id="chart_container" style="max-width: 1100px; min-width: <?php echo $chart_width; ?>px; height: <?php echo $chart_height; ?>px; margin: 0 auto"></div>
      <?php } ?>
        ...
        <tr>
          <td>
            <table>
              <tr>
                <td>
                  <input type="text" name="product_name" placeholder="<?php echo $language->get("text_filter_product");?>" autocomplete="off" value="<?php echo isset($product_name)?$product_name:"";?>" size="20"/>
      ...
      <?php if ($reports) { ?>
        <div class="table-responsive">
          <table class="list list-reports table table-bordered table-hover">
            <thead>
              <tr>
                 <td class="left" style="width:30%"><?php echo $column_period; ?></td>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="right"><?php echo isset($report["orders"])?$report['orders']:0;?></td>
            ...
            <tfoot>
              <tr>
                ...
                <td class="right"><b><?php echo $sum_qty; ?></b></td>
...
function ecExport() {
  url = 'index.php?route=ecadvancedreports/product/export&token=<?php echo $token; ?>';
...
function filter() {
  url = 'index.php?route=ecadvancedreports/product&token=<?php echo $token; ?>';
 ...
 google.load("visualization", "1", {packages:["corechart"]});
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['<?php echo $language->get("text_date");?>','<?php echo ($filter_reload_key=="qty")?$text_quantity:$text_total; ?>'],
          <?php
          $tmp = array();
          if($filter_reload_key == "qty") {
            foreach($reports as $key=>$val) {
               if($val) {
                  $tmp[] = "['".$key."', ".(isset($val["orders"])?$val["orders"]:"0")."]";
                }
            }
          } else {
            foreach($reports as $key=>$val) {
              if($val) {
                  $tmp[] = "['".$key."', ".(isset($val["total"])?$val["total"]:"0")."]";
              }
            }
          }
        ...
        var options = {
          ...
          hAxis: {title: '<?php echo $language->get("text_product_name")." ".(isset($product_name)?$product_name:""); ?>', titleTextStyle: {color: 'red'}}
        };
