          ...
          <div class="tab-content">
                ...
                <div class="tab-pane active" id="tab-product">
                  <fieldset>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <!--<input type="text" name="product_name" value="" id="input-product" class="form-control" />-->
                        <textarea type="text" name="product_name" id="input-product" class="form-control"></textarea>
                        <input type="hidden" class="product_id" name="product_id" value="" />
                      </div>
                    </div>
                    <div class="form-group">
                      <label class="col-sm-2 control-label" for="input-product_price"><?php echo "Product Price"; ?></label>
                      <div class="col-sm-10">
                        <input type="text" name="product_price" value="" id="input-product_price" class="form-control" />
                      </div>
                    </div>
                    <div class="form-group">
            ...
            <div class="tab-pane" id="tab-total">
              ...
              <fieldset>
				...
				<div class="form-group">
                  <label class="col-sm-2 control-label" for="input-delivery_date"><?php echo $entry_delivery_date; ?></label>
					<div class="col-sm-10">
						<div class="input-group date">
							<input type="text" name="delivery_date" value="<?php echo $delivery_date ?>" data-date-format="YYYY-MM-DD" id="input-delivery_date" class="form-control" />
							<span class="input-group-btn">
								<button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button>
							</span>
						</div>
                ...
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-delivery_time"><?php echo $entry_delivery_time; ?></label>
                  <div class="col-sm-10">
		...
		success: function(json) {
			...
			if (json['products'].length) {
				for (i = 0; i < json['products'].length; i++) {
					...
					html += '  <input type="hidden" name="product[' + i + '][name]" value="' + product['name'] + '" />';
					html += '  <input type="hidden" name="product[' + i + '][price]" value="' + product['ori_price'] + '" />';
...
$('#tab-product textarea[name=\'product_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
	...
	'select': function(item) {
		$('#tab-product textarea[name=\'product_name\']').val(item['label']);
		$('#tab-product input[name=\'product_price\']').val(item['price']);
			...
			for (i = 0; i < item['option'].length; i++) {
				...
				if (option['type'] == 'quantity') {
					...
					for (j = 0; j < option['product_option_value'].length; j++) {
						...
						html += '<div class="checkbox">';
							html += '<label class="col-sm-2 control-label" for="input-option' + option['product_option_id'] + '">'+option_value['name'];
							...
							html += '</label>';
							html += '<div class="col-sm-10"><input class="form-control" type="text" name="option[' + option['product_option_id'] + '][' + option_value['product_option_value_id'] + '][]" /></div>';
						html += '</div>';
					}
...
$('#button-product-add').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/cart/add&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
		type: 'post',
		data: $('#tab-product input[name=\'product_id\'], #tab-product textarea[name=\'product_name\'], #tab-product input[name=\'product_price\'], #tab-product input[name=\'quantity\'], #tab-product input[name^=\'option\'][type=\'text\'], #tab-product input[name^=\'option\'][type=\'hidden\'], #tab-product input[name^=\'option\'][type=\'radio\']:checked, #tab-product input[name^=\'option\'][type=\'checkbox\']:checked, #tab-product select[name^=\'option\'], #tab-product textarea[name^=\'option\']'),
		dataType: 'json',
		crossDomain: true,
		beforeSend: function() {
			$('#button-product-add').button('loading');
...
$('#cart').delegate('.btn-primary', 'click', function() {
    ...
    $.ajax({
        url: '<?php echo $catalog; ?>index.php?route=api/cart/add&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
        type: 'post',
        data: $('#cart input[name^=\'product\'][type=\'text\'], #cart input[name^=\'product\'][type=\'hidden\'], #cart input[name^=\'name\'][type=\'hidden\'], #cart input[name^=\'price\'][type=\'hidden\'], #cart input[name^=\'product\'][type=\'radio\']:checked, #cart input[name^=\'product\'][type=\'checkbox\']:checked, #cart select[name^=\'product\'], #cart textarea[name^=\'product\']'),
        dataType: 'json',
        crossDomain: true,
        beforeSend: function() {
            $(node).button('loading');
...
$('#button-coupon').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/coupon&token=' + token + '&order_id=<?= $order_id; ?>&store_id=' + $('select[name=\'store_id\'] option:selected').val() + '&order_id='+$('input[name=\'order_id\']').val(),
		type: 'post',
...
$('#button-voucher').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/voucher&order_id=<?= $order_id; ?>&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
		type: 'post',
...
$('#button-reward').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/reward&order_id=<?= $order_id; ?>&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
		type: 'post',
...
$('#button-save').on('click', function() {
	...
	$.ajax({
		url: url,
		type: 'post',
		data: $('select[name=\'payment_method\'] option:selected,  select[name=\'shipping_method\'] option:selected,  #tab-total select[name=\'order_status_id\'], #tab-total select, #tab-total textarea[name=\'comment\'], #tab-total input[name=\'delivery_date\'], #tab-total input[name=\'delivery_time\'], #tab-total input[name=\'affiliate_id\']'),
		dataType: 'json',
		crossDomain: true,
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				if ($('input[name=\'order_id\']').val() == 0) {
					//Add order flow, redirect to order info page
					window.location = 'index.php?route=sale/order/info&order_id='+json['order_id']+'&token=<?php echo $token; ?>';
				}
      }
